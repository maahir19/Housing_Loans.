...
def main():
    ...
    tab1, tab2, tab3, tab4, tab5, tab6 = st.tabs(["Model Performance","Score New Application","Feature Importance","Inspect Predictions","Model Comparison","KYC Verification"])
...
Divided by actual income: Rs{actual_credit:,}/mo
        ...
        if submitted:
            ...
            # Due-diligence findings - calibrated to the ACTUAL dose-response in the
            # data: 0 flags=2.55% fraud, 1=4.15%, 2=6.87%, 3=12.44%, 4=16.67% (n=6,
            # thin sample - the 4-flag tier is judgment-extrapolated).
            # HYBRID: pure floor could go inert (silently absorbed by an already-
            # elevated baseline); pure additive overshoots the calibrated target
            # when baseline is already high (e.g. base 35% + dd_adder 40% = 75%,
            # way above what 2 flags' real ~15% co-occurrence rate supports).
            # So: floor sets the calibrated target, PLUS a small guaranteed nudge
            # (+4 pts/flag) that always applies so it's never fully inert either.
            dd_floor = {0: 0.0, 1: 0.25, 2: 0.40, 3: 0.55}.get(due_diligence_critical_count, 0.65)
