...
def get_sandbox_token():
    try:
        r = requests.post(
            ...
            headers={
                "x-api-key": st.secrets["SANDBOX_API_KEY"],
                "x-api-secret": st.secrets["SANDBOX_API_SECRET"],
                "x-api-version": "1.0"
            },
            timeout=10
        )
...
def verify_pan_sandbox(pan, name, dob, token):
    r = requests.post(
...
def request_aadhaar_otp(aadhaar, token):
    r = requests.post(
...
def verify_aadhaar_otp(ref_id, otp, token):
    r = requests.post(
        ...
        json={"@entity": "in.co.sandbox.kyc.aadhaar.okyc.otp.verify.request", "ref_id": ref_id, "otp": otp},
        timeout=10
    )
    return r.json()
...
Divided by actual income: Rs{actual_credit:,}/mo
    ...
    with tab6:
        ...
        st.markdown('<p style="color:#8899aa;font-size:0.9rem">Verify borrower identity via Sandbox KYC APIs (PAN + Aadhaar) before processing the loan application.</p>', unsafe_allow_html=True)
        ...
        with kyc_tab1:
            ...
            st.markdown('<p style="color:#8899aa;font-size:0.85rem">Enter the borrower\'s PAN number to verify against Income Tax database.</p>', unsafe_allow_html=True)
            pan_col1, pan_col2 = st.columns([2, 1])
            with pan_col1:
                pan_input = st.text_input("PAN Number", placeholder="XXXPX1234A", max_chars=10,
                                          help="Format: 5 letters, 4 digits, 1 letter")
            with pan_col2:
                pan_name = st.text_input("Name as per PAN", placeholder="John Ronald Doe")
            pan_dob = st.text_input("Date of Birth", placeholder="11/11/2001", help="Format: DD/MM/YYYY")
            ...
            if verify_pan_btn:
                if not pan_input or len(pan_input) != 10:
                    ...
                    st.warning("Please enter name and date of birth as per PAN.")
                else:
                    with st.spinner("Connecting to Sandbox KYC..."):
                        token, err = get_sandbox_token()
                        if err or not token:
                            st.error(f"Authentication failed. Check your API keys in secrets.toml. Details: {err}")
                        else:
                            result = verify_pan_sandbox(pan_input.upper(), pan_name, pan_dob, token)
                            status = result.get("data", {}).get("status", "")
                            name   = result.get("data", {}).get("pan", "N/A")
                            category = result.get("data", {}).get("category", "N/A")

                            if status == "valid" or result.get("code") == 200:
                                st.markdown(f"""
                                <div style="background:#064e3b;border:1px solid #34d399;border-radius:12px;padding:1.25rem 1.5rem;margin:1rem 0">
                                    ...
                                    <div style="color:#6ee7b7;font-size:0.9rem">PAN: <strong style="color:#f0f4f8">{name}</strong></div>
                                    <div style="color:#6ee7b7;font-size:0.9rem;margin-top:4px">Category: <strong style="color:#f0f4f8">{category}</strong></div>
                                    <div style="color:#6ee7b7;font-size:0.9rem;margin-top:4px">Status: <strong style="color:#f0f4f8">{status}</strong></div>
                                </div>""", unsafe_allow_html=True)
                            else:
                                st.markdown(f"""
                                <div style="background:#7f1d1d;border:1px solid #f87171;border-radius:12px;padding:1.25rem 1.5rem;margin:1rem 0">
                                    <div style="font-size:1rem;font-weight:600;color:#f87171;margin-bottom:0.5rem">❌ PAN Verification Failed</div>
                                    <div style="color:#fca5a5;font-size:0.9rem">The PAN could not be verified. It may be invalid or not found in the database.</div>
                                </div>""", unsafe_allow_html=True)
        ...
        with kyc_tab2:
            ...
            with aadh_col1:
                aadhaar_input = st.text_input("Aadhaar Number", placeholder="XXXX XXXX XXXX", max_chars=14,
            ...
            if send_otp_btn:
                ...
                if len(aadhaar_clean) != 12 or not aadhaar_clean.isdigit():
                    ...
                    with st.spinner("Sending OTP to registered mobile..."):
                        token, err = get_sandbox_token()
                        if err or not token:
                            st.error(f"Authentication failed. Check your API keys. Details: {err}")
                        else:
                            otp_result = request_aadhaar_otp(aadhaar_clean, token)
                            ref_id = otp_result.get("data", {}).get("ref_id") or otp_result.get("ref_id")
                            if ref_id:
                                st.session_state.aadhaar_ref_id = ref_id
                                st.success("OTP sent successfully to the Aadhaar-registered mobile number.")
                            else:
                                st.error("Failed to send OTP. Check the Aadhaar number and try again.")
                                with st.expander("Raw API Response"):
                                    st.json(otp_result)
            ...
            if st.session_state.aadhaar_ref_id:
                ...
                with otp_col1:
                    otp_input = st.text_input("Enter OTP", placeholder="6-digit OTP", max_chars=6, type="password")
                with otp_col2:
                    st.markdown("<br>", unsafe_allow_html=True)
                ...
                if verify_otp_btn:
                    if not otp_input or len(otp_input) != 6:
                        ...
                        with st.spinner("Verifying OTP..."):
                            token, err = get_sandbox_token()
                            if err or not token:
                                st.error(f"Authentication failed. Details: {err}")
                            else:
                                verify_result = verify_aadhaar_otp(st.session_state.aadhaar_ref_id, otp_input, token)
                                data = verify_result.get("data", {})
                                name = data.get("name", "N/A")
                                dob  = data.get("dob", "N/A")
                                gender = data.get("gender", "N/A")
                                address = data.get("address", {})
                                addr_str = ", ".join([v for v in address.values() if v]) if isinstance(address, dict) else str(address)

                                if verify_result.get("code") == 200 or name != "N/A":
                                    ...
                                    <div style="background:#064e3b;border:1px solid #34d399;border-radius:12px;padding:1.25rem 1.5rem;margin:1rem 0">
                                        ...
                                        <div style="color:#6ee7b7;font-size:0.9rem">Name: <strong style="color:#f0f4f8">{name}</strong></div>
                                        <div style="color:#6ee7b7;font-size:0.9rem;margin-top:4px">Date of Birth: <strong style="color:#f0f4f8">{dob}</strong></div>
                                        <div style="color:#6ee7b7;font-size:0.9rem;margin-top:4px">Gender: <strong style="color:#f0f4f8">{gender}</strong></div>
                                        <div style="color:#6ee7b7;font-size:0.9rem;margin-top:4px">Address: <strong style="color:#f0f4f8">{addr_str}</strong></div>
                                    </div>""", unsafe_allow_html=True)
                                    st.session_state.aadhaar_ref_id = None
                                else:
                                    st.markdown("""
                                    <div style="background:#7f1d1d;border:1px solid #f87171;border-radius:12px;padding:1.25rem 1.5rem;margin:1rem 0">
                                        <div style="font-size:1rem;font-weight:600;color:#f87171;margin-bottom:0.5rem">❌ OTP Verification Failed</div>
                                        <div style="color:#fca5a5;font-size:0.9rem">Incorrect or expired OTP. Please try again.</div>
                                    </div>""", unsafe_allow_html=True)
                                with st.expander("Raw API Response"):
        ...
        st.markdown("---")
        st.markdown("""
        <div style="background:#1e3a5f;border:1px solid #1d4ed8;border-radius:12px;padding:1rem 1.5rem">
            <div style="font-size:0.72rem;font-weight:600;color:#60a5fa;text-transform:uppercase;letter-spacing:0.1em;margin-bottom:0.5rem">SANDBOX TEST CREDENTIALS</div>
            <p style="color:#cbd5e1;font-size:0.88rem;margin:0;line-height:1.8">
            <strong style="color:#f0f4f8">PAN (test):</strong> XXXPX1234A &nbsp;|&nbsp; Name: John Ronald Doe &nbsp;|&nbsp; DOB: 11/11/2001<br>
            <strong style="color:#f0f4f8">Aadhaar (test):</strong> 123456789012
            </p>
        </div>""", unsafe_allow_html=True)
