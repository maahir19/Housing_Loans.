...
def build_system_prompt(inputs, score, inflate, flags_count, shap_summary, overall_risk=None):
    ...
    risk_level = "HIGH RISK" if display_score >= 0.70 else "MODERATE RISK" if display_score >= 0.40 else "LOW RISK"
...
You are a senior colleague. Give concise bullet answers max 200 words. Recommend APPROVE/REJECT/INVESTIGATE when asked. Ground answers in the numbers above. You know RBI guidelines and Indian housing loan fraud patterns."""
...
def main():
    ...
    with tab2:
        ...
        if submitted:
            ...
            # CIBIL: below 650 is bad. Weight: high.
            cibil_r = sigmoid_risk(cibil, 620, 50, increasing=False)
            risk_components.append(("CIBIL score", 2.5, cibil_r,
                f"CIBIL is {cibil} ({'below' if cibil < 650 else 'above'} the 650 safe minimum)."))

            # Effective FOIR after new loan. Threshold 60%.
            eff_foir_r = sigmoid_risk(effective_foir, 0.60, 0.08, increasing=True)
            risk_components.append(("FOIR after this loan", 2.0, eff_foir_r,
                f"Total obligations after this loan: {effective_foir*100:.0f}% of actual income (cap: 60%)."))

            # EMI affordability: new EMI as % of actual income. Threshold 40%.
            emi_inc_r = sigmoid_risk(emi_to_income, 0.40, 0.06, increasing=True)
            risk_components.append(("EMI affordability", 2.0, emi_inc_r,
                f"New EMI is {emi_to_income*100:.0f}% of actual monthly income."))

            # Loan-to-income: loan vs annual actual income. >5x is high risk.
            annual_actual = actual_credit_safe * 12
            lti = loan_amount / annual_actual if annual_actual > 0 else 0
            lti_r = sigmoid_risk(lti, 5.0, 1.0, increasing=True)
            risk_components.append(("Loan-to-income ratio", 2.0, lti_r,
                f"Loan is {lti:.1f}x annual income (safe limit: 5x)."))

            # LTV: only risky above 80%.
            ltv_r = sigmoid_risk(ltv, 0.80, 0.05, increasing=True)
            risk_components.append(("Loan-to-value", 1.0, ltv_r,
                f"LTV is {ltv*100:.0f}% (safe limit: 80%)."))

            # Age at loan end: risky if borrower > 65 at maturity.
            age_tenure_r = sigmoid_risk(age + tenure, 65, 3, increasing=True)
            risk_components.append(("Age at loan end", 1.0, age_tenure_r,
                f"Borrower will be {age + tenure} at loan maturity (limit: 65)."))

            # Employment stability: < 1 year is a red flag.
            emp_years_r = sigmoid_risk(years_employer, 1.0, 0.8, increasing=False)
            risk_components.append(("Employment stability", 1.0, emp_years_r,
                f"{years_employer} year(s) with current employer (minimum: 1 year)."))

            # Income inflation: stated vs actual bank credits.
            inflate_r = min(sigmoid_risk(inflate, 1.4, 0.15, increasing=True), 0.80)
            risk_components.append(("Income consistency", 1.5, inflate_r,
                f"Stated income is {inflate:.2f}x actual bank credits."))
            ...
            critical_doc_risk = min(n_critical * 0.25, 0.80)
            minor_doc_risk    = min(n_minor * 0.05, 0.30)
            doc_risk = min(critical_doc_risk + minor_doc_risk, 0.90)
