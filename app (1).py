...
def load_and_train():
    ...
    try:
        ...
        if hasattr(best_model, 'estimators_'):
            all_imp = []
            for _, est in best_model.estimators_:
...
Divided by actual income: Rs{actual_credit:,}/mo
        ...
        if submitted:
            ...
            risk_components = [
                ...
                ("Employment stability",2.0, _t_emp(years_employer),
                    f"{years_employer} year(s) with employer (minimum: 2 years for salaried)."),
                ("Age at maturity",     2.0, _t_age(age_at_end),
            ...
            # Employment
            if years_employer == 0:  overall_risk = max(overall_risk, 0.55)
            elif years_employer < 1: overall_risk = max(overall_risk, 0.40)
            elif years_employer < 2: overall_risk = max(overall_risk, 0.25)

            # Age at maturity
            if age_at_end > 70:   overall_risk = max(overall_risk, 0.55)
            elif age_at_end > 65: overall_risk = max(overall_risk, 0.40)
            elif age_at_end > 60: overall_risk = max(overall_risk, 0.25)
            ...
            # Employment
            if years_employer == 0:
                breach_notes.append(f"EMPLOYMENT THRESHOLD BREACHED: 0 years with current employer. Minimum requirement is 2 years for salaried applicants.")
            elif years_employer < 1:
                breach_notes.append(f"EMPLOYMENT THRESHOLD BREACHED: {years_employer} year(s) with employer - below the 1-year minimum. Confirm job stability and offer letter.")
            elif years_employer < 2:
                breach_notes.append(f"EMPLOYMENT WARNING: {years_employer} year(s) with employer is below the preferred 2-year threshold for salaried borrowers.")

            # Age at maturity
            if age_at_end > 70:
                breach_notes.append(f"AGE AT MATURITY BREACHED: Borrower will be {age_at_end} at loan end. Most lenders cap salaried maturity at 60, self-employed at 65.")
            elif age_at_end > 65:
                breach_notes.append(f"AGE AT MATURITY WARNING: Borrower will be {age_at_end} at loan end - above the 60-year salaried cap. Confirm employment type eligibility.")
            elif age_at_end > 60:
                breach_notes.append(f"AGE AT MATURITY ELEVATED: Borrower will be {age_at_end} at loan end. Salaried limit is 60 - verify post-retirement income source.")
