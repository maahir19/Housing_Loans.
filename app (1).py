...
def load_and_train():
    ...
    try:
        # Use SMOTE-augmented training data consistent with how the model was trained
        kf_scores = cross_val_score(best_model, X_train_sm if is_tree else X_train_s_sm, y_train_sm, cv=kf, scoring='roc_auc')
    except Exception:
...
Divided by actual income: Rs{actual_credit:,}/mo
        ...
        with btn_col:
            if st.button("Calculate Fraud Risk Score"):
                st.session_state['scoring_active'] = True

        if st.session_state.get('scoring_active', False):
            submitted = True
        else:
            submitted = False
        ...
        if submitted:
            ...
            risk_components = [
                ...
                ("FOIR after this loan",3.0, _t_foir(effective_foir),
                    f"Total obligations: {effective_foir*100:.0f}% of actual income (RBI cap: 50-60%)."),
            ...
            # ── Hard policy floors - must match breach alert severity ────────
            # Rule: if a breach alert fires at a certain severity, the score
            # cannot sit below that band. No more contradictions.

            # FOIR
            if effective_foir > 0.75:   overall_risk = max(overall_risk, 0.70)  # critically over-leveraged = CRITICAL RISK
            elif effective_foir > 0.65: overall_risk = max(overall_risk, 0.55)  # breaches RBI cap = HIGH RISK
            elif effective_foir > 0.55: overall_risk = max(overall_risk, 0.40)  # elevated = MEDIUM RISK

            # LTV
            if ltv > 0.80:   overall_risk = max(overall_risk, 0.55)  # breaches RBI cap = HIGH RISK
            elif ltv > 0.75: overall_risk = max(overall_risk, 0.40)  # warning = MEDIUM RISK

            # Income inflation
            if inflate > 2.0:   overall_risk = max(overall_risk, 0.70)  # critical inflation = CRITICAL RISK
            elif inflate > 1.5: overall_risk = max(overall_risk, 0.55)  # breaches safe limit = HIGH RISK
            elif inflate > 1.2: overall_risk = max(overall_risk, 0.40)  # warning = MEDIUM RISK

            # Loan-to-income
            if lti > 6.0:   overall_risk = max(overall_risk, 0.55)  # breaches safe limit = HIGH RISK
            elif lti > 5.0: overall_risk = max(overall_risk, 0.40)  # elevated = MEDIUM RISK

            # CIBIL
            if cibil < 600:   overall_risk = max(overall_risk, 0.70)  # critically low = CRITICAL RISK
            elif cibil < 650: overall_risk = max(overall_risk, 0.55)  # below minimum = HIGH RISK
            elif cibil < 700: overall_risk = max(overall_risk, 0.40)  # below preferred = MEDIUM RISK

            # Employment
            if years_employer == 0:    overall_risk = max(overall_risk, 0.55)  # zero tenure = HIGH RISK
            elif years_employer < 1:   overall_risk = max(overall_risk, 0.40)  # below minimum = MEDIUM RISK
            elif years_employer < 2:   overall_risk = max(overall_risk, 0.25)  # below preferred = LOW-MEDIUM

            # Age at maturity
            if age_at_end > 70:   overall_risk = max(overall_risk, 0.55)  # breaches hard cap = HIGH RISK
            elif age_at_end > 65: overall_risk = max(overall_risk, 0.40)  # above salaried cap = MEDIUM RISK
            elif age_at_end > 60: overall_risk = max(overall_risk, 0.25)  # elevated = LOW-MEDIUM

            # Credit enquiries
            if enquiries > 6:   overall_risk = max(overall_risk, 0.55)  # critical = HIGH RISK
            elif enquiries > 3: overall_risk = max(overall_risk, 0.40)  # elevated = MEDIUM RISK
            elif enquiries > 2: overall_risk = max(overall_risk, 0.25)  # worth noting = LOW-MEDIUM
            ...
            # FOIR
            if foir > 0.75:
                breach_notes.append(f"FOIR CRITICALLY HIGH: {foir*100:.0f}% - borrower is mathematically over-leveraged. RBI absolute cap is 60%. Disbursement not advisable.")
            elif foir > 0.65:
                ...
                breach_notes.append(f"FOIR ELEVATED: {foir*100:.0f}% is above the 50-55% comfortable range. Borrower has limited repayment buffer.")
