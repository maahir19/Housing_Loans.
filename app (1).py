...
Divided by actual income: Rs{actual_credit:,}/mo
        ...
        if submitted:
            ...
            # Due-diligence findings - calibrated to their ACTUAL fraud rate in the
            # data (~15% for 2+ findings together, ~5-7% for one alone), not the
            # much higher rate that justifies the document-tampering floor. This
            # lands them in "verify/escalate" territory rather than "reject."
            if due_diligence_critical_count >= 2:
                overall_risk = max(overall_risk, 0.42)
            elif due_diligence_critical_count == 1:
                overall_risk = max(overall_risk, 0.27)
            ...
            if due_diligence_critical_count > 0:
                ...
                dd_display_score = 0.42 if due_diligence_critical_count >= 2 else 0.27
