...
def load_and_train():
    ...
    xgb  = XGBClassifier(n_estimators=100, scale_pos_weight=50, random_state=42, eval_metric='logloss', verbosity=0)
    ...
    voting_xgb_rf = VotingClassifier(
        estimators=[('xgb', XGBClassifier(n_estimators=100, scale_pos_weight=50, random_state=42, eval_metric='logloss', verbosity=0)),
                    ('rf',  RandomForestClassifier(n_estimators=100, class_weight='balanced', random_state=42))],
        voting='soft')
    ...
    comparison_rows = []
    best_recall, best_model, best_name = -1, None, ''
    ...
    for name, clf in candidates.items():
        ...
        try:
            ...
            comparison_rows.append({'Model':name,'Type':tag,'AUC':round(m_auc,4),'Recall':round(m_rec,4),'Precision':round(m_prec,4),'F1':round(m_f1,4)})
            if m_rec > best_recall:
                best_recall, best_model, best_name = m_rec, clf, name
        except Exception as e:
            comparison_rows.append({'Model':name,'Type':'Error','AUC':0,'Recall':0,'Precision':0,'F1':0})
    ...
    try:
        ...
        lr_fb.fit(X_train_s, y_train)
        explainer = shap.LinearExplainer(lr_fb, X_train_s, feature_perturbation="interventional")
        use_tree_shap = False
...
def main():
    ...
    with tab2:
        ...
        if submitted:
            ...
            score = float(np.clip((score - 0.35) / (1.0 - 0.35), 0.0, 1.0))  # recalibrate: treat <0.35 as no signal
            ...
            emi_inc_r = sigmoid_risk(emi_to_income, 0.50, 0.07, increasing=True)
            ...
            ltv_r = sigmoid_risk(ltv, 0.85, 0.05, increasing=True)
            ...
            age_tenure_r = sigmoid_risk(age + tenure, 75, 5, increasing=True)
            risk_components.append(("Age at loan end", age_tenure_r,
                f"The borrower would be {age + tenure} when this loan is fully repaid."))

            emp_years_r = sigmoid_risk(years_employer, 1.5, 1.0, increasing=False)
            risk_components.append(("Employment history", emp_years_r,
                f"{years_employer} years with the current employer."))
