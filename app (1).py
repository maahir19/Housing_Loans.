...
def main():
    ...
    with tab2:
        ...
        if submitted:
            ...
            # Each feature maps to a tier 0-4: clean/caution/medium/high/critical
            # Tier scores: 0→0.08, 1→0.28, 2→0.48, 3→0.68, 4→0.88
            # Overall = 60% worst feature + 40% weighted mean
            # One terrible feature genuinely dominates — not averaged away.
            ...
            # Critical flags: each adds 0.25, uncapped (stacks — 2 criticals = 0.50)
            # Minor flags: each adds 0.04, hard cap at 0.20 (5 minor flags = max)
            # This means minor flags alone can push to medium risk but never high/critical
            critical_doc_risk = min(n_critical * 0.25, 1.0)
            minor_doc_risk    = min(n_minor * 0.04, 0.20)
            doc_risk = min(critical_doc_risk + minor_doc_risk, 1.0)

            # Blend: 70% credit risk (tier-based), 30% doc flags
            # ML model score removed from blend — tier engine is now authoritative
            if n_critical > 0 or n_minor > 0:
            ...
            if cibil < 600 and overall_risk < 0.55:
                overall_risk = 0.55
            # Employment < 1 year: minimum low-medium risk
            if years_employer < 1 and overall_risk < 0.25:
                overall_risk = 0.25
            # Employment = 0 years: minimum medium risk (no job history at all)
            ...
            # ── Flag stacking: floors + additive minor flag penalty ───────────
            ...
            # Step 1: apply critical flag floor
            # 1 critical  → floor 0.55
            # 2+ critical → floor 0.70
            ...
            # Step 2: each minor flag adds 0.03 ON TOP of whatever score is already there
            # This means minor flags stack visibly — they can push from 0.55 up toward 0.70+
            # Cap: minor flags alone can add max 0.18 (6 flags × 0.03)
            minor_flag_adder = min(n_minor * 0.03, 0.18)
            overall_risk = min(overall_risk + minor_flag_adder, 1.0)

            # Step 3: any risk flag at all → minimum 0.40
