...
def main():
    ...
    with tab2:
        ...
        if submitted:
            ...
            # ── CIBIL ────────────────────────────────────────────────────────
            # Scale widened to 60 so it doesn't saturate too fast at extremes
            cibil_r = sigmoid_risk(cibil, 680, 60, increasing=False)
            ...
            # Scale widened to 0.08 to avoid saturation above 65%
            eff_foir_r = sigmoid_risk(effective_foir, 0.50, 0.08, increasing=True)
            ...
            # ── EMI affordability ────────────────────────────────────────────
            # Scale widened to 0.08 to avoid saturation above 50%
            emi_inc_r = sigmoid_risk(emi_to_income, 0.35, 0.08, increasing=True)
            ...
            # ── Loan-to-income ───────────────────────────────────────────────
            # Scale widened to 1.2 to spread the curve across 3-8x range
            annual_actual = actual_credit_safe * 12
            lti = loan_amount / annual_actual if annual_actual > 0 else 0
            lti_r = sigmoid_risk(lti, 4.5, 1.2, increasing=True)
            risk_components.append(("Loan-to-income ratio", 2.5, lti_r,
                f"Loan is {lti:.1f}x annual income (safe limit: 5x)."))

            # ── LTV ──────────────────────────────────────────────────────────
            # Scale widened to 0.06 to avoid saturation above 85%
            ltv_r = sigmoid_risk(ltv, 0.73, 0.06, increasing=True)
            ...
            age_tenure_r = sigmoid_risk(age_at_end, 60, 3, increasing=True)
            ...
            # Scale widened to 1.2 so 0 yrs vs 1 yr vs 3 yrs are meaningfully different
            emp_years_r = sigmoid_risk(years_employer, 2.0, 1.2, increasing=False)
            ...
            # ── Income inflation ─────────────────────────────────────────────
            # Scale widened to 0.20 to spread 1.0x-2.0x meaningfully
            inflate_r = sigmoid_risk(inflate, 1.3, 0.20, increasing=True)
            risk_components.append(("Income inflation", 2.5, inflate_r,
                f"Stated income is {inflate:.2f}x actual bank credits (safe limit: up to 1.2x)."))

            # ── Credit enquiries ─────────────────────────────────────────────
            enq_r = sigmoid_risk(enquiries, 3, 1.5, increasing=True)
            ...
            # Hard floor per critical flag:
            # 1 critical flag  → minimum 0.55 (HIGH RISK - DO NOT DISBURSE)
            # 2+ critical flags → minimum 0.70 (CRITICAL RISK - REJECT & FLAG)
            ...
            # Any risk flag (even non-critical): minimum 0.40
