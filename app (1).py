...
def load_and_train():
    ...
    xgb  = XGBClassifier(n_estimators=100, scale_pos_weight=24, random_state=42, eval_metric='logloss', verbosity=0)
    ...
    voting_xgb_rf = VotingClassifier(
        estimators=[('xgb', XGBClassifier(n_estimators=100, scale_pos_weight=24, random_state=42, eval_metric='logloss', verbosity=0)),
                    ('rf',  RandomForestClassifier(n_estimators=100, class_weight='balanced', random_state=42))],
        voting='soft')
    ...
    for name, clf in candidates.items():
        Xtr = X_train if name in TREE_MODELS else X_train_s
        ...
        try:
            ...
            if m_auc > best_auc:
                best_auc, best_model, best_name = m_auc, clf, name
