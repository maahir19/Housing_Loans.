...
def verify_pan_sandbox(pan, token):
    r = requests.post(
...
def request_aadhaar_otp(aadhaar, token):
    r = requests.post(
        ...
        json={"aadhaar_number": aadhaar, "consent": "y"},
        timeout=10
    )
    return r.json()
...
Divided by actual income: Rs{actual_credit:,}/mo
    ...
    with tab6:
        ...
        with kyc_tab1:
            ...
            with pan_col1:
                pan_input = st.text_input("PAN Number", placeholder="ABCDE1234F", max_chars=10,
                ...
                if not pan_input or len(pan_input) != 10:
                    ...
                    with st.spinner("Connecting to Sandbox KYC..."):
                        ...
                        if err or not token:
                            ...
                            result = verify_pan_sandbox(pan_input.upper(), token)
                            status = result.get("data", {}).get("pan_status", "")
                            name   = result.get("data", {}).get("name", "N/A")
                            pan_type = result.get("data", {}).get("pan_type", "N/A")
                            ...
                            if status == "VALID" or result.get("code") == 200:
                                ...
                                <div style="background:#064e3b;border:1px solid #34d399;border-radius:12px;padding:1.25rem 1.5rem;margin:1rem 0">
                                    ...
                                    <div style="color:#6ee7b7;font-size:0.9rem;margin-top:4px">PAN Type: <strong style="color:#f0f4f8">{pan_type}</strong></div>
        ...
        <div style="background:#1e3a5f;border:1px solid #1d4ed8;border-radius:12px;padding:1rem 1.5rem">
            ...
            <strong style="color:#f0f4f8">PAN (valid):</strong> ABCDE1234F &nbsp;|&nbsp;
            <strong style="color:#f0f4f8">PAN (invalid):</strong> ZZZZZ9999Z<br>
            <strong style="color:#f0f4f8">Aadhaar:</strong> Use test numbers from your Sandbox dashboard under the KYC section.
            </p>
        </div>""", unsafe_allow_html=True)
