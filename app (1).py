...
def load_and_train():
    ...
    # Additional pre-sanction due-diligence features. Verified against timestamps:
    # avg Application->Sanction gap is ~18 days, and Legal_Opinion_TAT_Hours averages
    # ~71 hours - well inside that window - so valuation/legal/title/CERSAI/RERA
    # checks are all completed and known BEFORE the sanction decision, making them
    # safe to use for a pre-sanction fraud score (no leakage).
    # NOTE: EMI_INR and Interest_Rate_pct are deliberately excluded - they are the
    # FINAL approved loan terms, only fixed at/after sanction, so using them here
    # would be circular (scoring on the outcome of the decision the score informs).
    df['valuation_market_gap'] = (df['Market_Value_INR'] - df['Loan_Amount_INR']) / df['Market_Value_INR'].replace(0,np.nan)
    ...
    categorical_features = ['Employment_Type','Loan_Purpose','Property_Type','Down_Payment_Source','Disbursement_Mode',
        'Valuation_Agency','RERA_Registered','CERSAI_Checked','Title_Chain_Clear','Legal_Opinion','Broker_Involved']
    ...
    # Vectorisation: fit a OneHotEncoder on the training categorical columns.
    # This replaces the bare pd.get_dummies call and ensures that:
    #   1. Unseen categories at inference time are handled gracefully (handle_unknown='ignore').
    #   2. The exact same columns in the exact same order are produced every time,
    #      rather than depending on whatever categories that happen to appear in a single row.
    ohe = OneHotEncoder(drop='first', sparse_output=False, handle_unknown='ignore')
    ohe.fit(X[categorical_features])
    ohe_cols = ohe.get_feature_names_out(categorical_features)
    ohe_array = ohe.transform(X[categorical_features])
    X = X.drop(columns=categorical_features)
    X = pd.concat([X.reset_index(drop=True),
                   pd.DataFrame(ohe_array, columns=ohe_cols)], axis=1)
    ...
    # SMOTE must run on SCALED data only. Raw feature magnitudes (Loan_Amount_INR
    # in millions vs LTV_Ratio 0-1) distort KNN distances and produce biased
    # synthetic samples. We SMOTE the scaled set, then inverse-transform for trees.
    sm = SMOTE(random_state=42, k_neighbors=3)
    X_train_s_sm, y_train_s_sm = sm.fit_resample(X_train_s, y_train)
    X_train_sm = pd.DataFrame(scaler.inverse_transform(X_train_s_sm), columns=X_train.columns)
    y_train_sm = y_train_s_sm
    ...
    for name, clf in candidates.items():
        ...
        try:
            ...
            thresh = find_best_threshold(y_test, prob, min_recall=0.80)
            ...
            if m_rec >= 0.80 and m_f1 > best_f1:
    ...
    try:
        # VotingClassifier has no .feature_importances_ directly — average across sub-models
        if hasattr(best_model, 'estimators_'):
            all_imp = []
            for est in best_model.estimators_:
                if hasattr(est, 'feature_importances_'):
                    all_imp.append(est.feature_importances_[:len(X_train.columns)])
            if all_imp:
    ...
    try:
        # Honest CV: SMOTE applied INSIDE each fold only on the training portion.
        # Previously SMOTE ran on the full training set before CV, which leaked
        # synthetic samples into validation folds and inflated scores to ~0.9999.
        # Now each fold sees only real data at validation time.
        if is_tree:
            cv_pipeline = ImbPipeline([
                ('smote', SMOTE(random_state=42, k_neighbors=3)),
                ('model', best_model)
            ])
            kf_scores = cross_val_score(cv_pipeline, X_train, y_train, cv=kf, scoring='roc_auc')
        else:
            cv_pipeline = ImbPipeline([
                ('smote', SMOTE(random_state=42, k_neighbors=3)),
                ('model', best_model)
            ])
            kf_scores = cross_val_score(cv_pipeline, X_train_s, y_train, cv=kf, scoring='roc_auc')
    except Exception:
    ...
    return (best_model, scaler, ohe, list(X_train.columns), auc, prec, rec,
...
def score_application(inputs, model, scaler, ohe, feature_names, numeric_features, categorical_features, doc_flag_cols, is_tree):
    ...
    market_value = inputs.get('Market_Value_INR', 0) or 0
    loan_amt_    = inputs.get('Loan_Amount_INR', 0) or 0
    app['valuation_market_gap'] = (market_value - loan_amt_) / market_value if market_value else 0
    for col in categorical_features:
        if col not in app.columns: app[col] = 'Unknown'

    # Vectorisation: use the fitted OneHotEncoder rather than pd.get_dummies.
    # pd.get_dummies on a single row only produces columns for categories present
    # in that row, so rare or unseen values produce a different column set each
    # time and require the fragile "add missing columns as 0" patch below.
    # The fitted encoder always outputs the same fixed column set regardless of
    # what categories appear in this specific row.
    ohe_cols   = ohe.get_feature_names_out(categorical_features)
    ohe_array  = ohe.transform(app[categorical_features])
    app = app.drop(columns=categorical_features)
    app = pd.concat([app.reset_index(drop=True),
                     pd.DataFrame(ohe_array, columns=ohe_cols)], axis=1)
...
def render_loan_table(data, category):
    ...
    for col in ['Monthly_Income_Stated','Actual_Bank_Credit_Monthly','Annual_Income_Stated','ITR_Income_Last_FY','Loan_Amount_INR','Market_Value_INR']:
...
def _t_emp(y, employment="Salaried"):
    # Salaried: job stability measured by tenure at current employer.
    # Self-employed: "years with employer" = years running the current
    # business/practice - lenders actually want a LONGER vintage here
    # since a new business is riskier than a new salaried job.
    # Government: greater job security, so a shorter minimum is acceptable.
    if employment in ("Self-Employed Business", "Self-Employed Professional"):
        if y >= 5:   return 0
        if y >= 3:   return 1
        if y >= 2:   return 2
        if y >= 1:   return 3
        return 4
    elif employment == "Government":
        if y >= 1:    return 0
        if y >= 0.5:  return 1
        if y >= 0.25: return 2
        return 3
    else:  # Salaried (default)
        if y >= 3:   return 0
        if y >= 2:   return 1
        if y >= 1:   return 2
        if y >= 0.5: return 3
...
def _t_age(a, employment="Salaried"):
    # Age-at-maturity cap varies by employment type: salaried/government
    # borrowers are capped near the standard retirement age (60), while
    # self-employed borrowers (no fixed retirement) get a higher cap (65).
    cap = 65 if employment in ("Self-Employed Business", "Self-Employed Professional") else 60
    if a <= cap - 5:  return 0
    if a <= cap:      return 1
    if a <= cap + 5:  return 2
    if a <= cap + 10: return 3
    return 4
...
def _t_enq(e):
    ...
    if e <= 4: return 2
    if e <= 6: return 3
    return 4  # 7+ also gets the hard floor below - this tier just feeds the blend

TIER_SCORES = [0.08, 0.28, 0.48, 0.68, 0.88]



#  Pytesseract OCR helper 
...
def parse_pan_from_text(text):
    """Extract PAN number, name and DOB from OCR text."""
    pan, name, dob = None, None, None
    ...
    # Name: line after "Name" keyword or before PAN
...
def main():
    (model, scaler, ohe, feature_names, auc, prec, rec,
     tn, fp, fn, tp, coef_df, kf_scores, df, doc_flag_cols,
     numeric_features, categorical_features, test_df, explainer,
    ...
    def page_score_application():
        ...
        with col1:
            ...
            MIN_WORKING_AGE = 18
            max_plausible_tenure = max(age - MIN_WORKING_AGE, 0)
            age_employer_inconsistent = years_employer > max_plausible_tenure
            if age_employer_inconsistent:
                st.error(f"⚠ Impossible combination: a {age}-year-old cannot have {years_employer} year(s) with their current employer "
                         f"— that would mean starting work at age {age - years_employer}. Max plausible tenure at this age is {max_plausible_tenure} year(s).")
        ...
        with col2:
            ...
            market_value  = st.number_input("Market Value of Property (Rs)", 500000, 100000000, 7500000, step=100000,
                                help="From the valuation report. LTV below is calculated from this, not entered separately.")
            ltv = min(loan_amount / market_value, 1.0) if market_value else 0.0
            st.markdown(f"""
<div style="background:#1e3a5f;border:1px solid #3b82f644;border-radius:10px;padding:0.75rem 1rem;margin-top:0.5rem;margin-bottom:0.75rem">
<div style="font-size:0.7rem;font-weight:600;color:#60a5fa;text-transform:uppercase;letter-spacing:0.08em;margin-bottom:6px">AUTO-CALCULATED LTV</div>
<div style="font-size:1.4rem;font-weight:700;color:#f0f4f8">{ltv*100:.1f}%</div>
<div style="font-size:0.82rem;color:#94a3b8;margin-top:4px">
Loan Amount: Rs{loan_amount:,} &nbsp;÷&nbsp; Market Value: Rs{market_value:,}
</div>
</div>""", unsafe_allow_html=True)
            tenure        = st.slider("Tenure (Years)", 1, 30, 15)
            emi           = st.number_input("Existing Monthly EMI (Rs) - from CIBIL report", 0, 500000, 0, step=1000,
                                help="Total of all current loan EMIs the borrower is already paying. Read directly off their CIBIL report.")
            _annual_rate   = 0.09
            _monthly_rate  = _annual_rate / 12
            _tenure_months = tenure * 12
            if _tenure_months > 0 and _monthly_rate > 0:
                _new_emi = loan_amount * _monthly_rate * (1 + _monthly_rate)**_tenure_months / ((1 + _monthly_rate)**_tenure_months - 1)
            else:
                _new_emi = 0
            _actual_safe = actual_credit if actual_credit > 0 else 1
            foir = min(round((emi + _new_emi) / _actual_safe, 4), 1.0)
            _total_obligations = emi + _new_emi
            st.markdown(f"""
<div style="background:#1e3a5f;border:1px solid #3b82f644;border-radius:10px;padding:0.75rem 1rem;margin-top:0.5rem">
<div style="font-size:0.7rem;font-weight:600;color:#60a5fa;text-transform:uppercase;letter-spacing:0.08em;margin-bottom:6px">AUTO-CALCULATED FOIR</div>
<div style="font-size:1.4rem;font-weight:700;color:#f0f4f8">{foir*100:.1f}%</div>
<div style="font-size:0.82rem;color:#94a3b8;margin-top:4px">
Existing obligations: Rs{emi:,}/mo &nbsp;+&nbsp; New EMI: Rs{_new_emi:,.0f}/mo &nbsp;=&nbsp; Rs{_total_obligations:,.0f}/mo<br>
Divided by actual income: Rs{actual_credit:,}/mo
</div>
</div>""", unsafe_allow_html=True)
        with col3:
            ...
            st.markdown("**Due-Diligence (from underwriting file)**")
            valuation_agency = st.selectbox("Valuation Agency", ["Empanelled Valuer A","Empanelled Valuer B","Third Party Valuer","Collusive Valuer"])
            rera_registered  = st.selectbox("RERA Registered", ["Yes","No","Not Applicable"])
            cersai_checked   = st.selectbox("CERSAI Checked", ["Yes","No","Post-Sanction Only"])
            title_chain      = st.selectbox("Title Chain Clear", ["Yes","Partial","No"])
            legal_opinion    = st.selectbox("Legal Opinion", ["Clear","Minor Observations","Conditional Clear","Adverse"])
            broker_involved  = st.selectbox("Broker Involved", ["No","Yes"])
        ...
        with btn_col:
            submitted = st.button("Calculate Fraud Risk Score")
        ...
        if submitted:
            inputs = {
                ...
                'Valuation_Agency':valuation_agency,'RERA_Registered':rera_registered,
                'CERSAI_Checked':cersai_checked,'Title_Chain_Clear':title_chain,
                'Legal_Opinion':legal_opinion,'Broker_Involved':broker_involved,
            ...
            score, app_df = score_application(inputs, model, scaler, ohe, feature_names, numeric_features, categorical_features, doc_flag_cols, is_tree)
            # Due-diligence-neutral score: same inputs, but due-diligence categorical
            # fields forced to their safest value. Used ONLY in the overall_risk blend
            # below, so due-diligence risk flows exclusively through the deterministic
            # floor/nudge system (guaranteed monotonic) rather than through the raw
            # model score, which can move non-monotonically on rare category
            # combinations it saw too few of in training (e.g. Collusive Valuer +
            # RERA=No together scored LOWER than Collusive Valuer alone - noise, not
            # signal). The real `score` is still shown as-is in the Model Performance
            # tab and reasons panel for transparency.
            inputs_dd_neutral = dict(inputs)
            inputs_dd_neutral.update({
                'Valuation_Agency': 'Empanelled Valuer A', 'RERA_Registered': 'Yes',
                'CERSAI_Checked': 'Yes', 'Title_Chain_Clear': 'Yes',
                'Legal_Opinion': 'Clear', 'Broker_Involved': 'No',
            })
            score_for_blend, _ = score_application(inputs_dd_neutral, model, scaler, ohe, feature_names, numeric_features, categorical_features, doc_flag_cols, is_tree)
            ...
            critical_flags_triggered = sum([
                ...
                disb_nonseller, pdf_anomaly, val_inflation
            ])
            # Due-diligence findings are real risk signals, but MUCH weaker than a
            # confirmed document-tampering flag. Checked against the data: tampering/
            # AML/signature-mismatch flags carry a 23-60% fraud rate when triggered,
            # whereas these due-diligence findings carry only 5-7% individually and
            # ~15% even when Collusive Valuer + severe Legal Opinion occur together
            # (vs a 4% baseline). So they get their own, much lower floor further
            # below - NOT the same 0.55/0.70 floor used for document tampering.
            valuation_collusive = (valuation_agency == "Collusive Valuer")
            legal_severe         = legal_opinion in ("Adverse", "Conditional Clear")
            rera_not_registered  = (rera_registered == "No")
            cersai_post_sanction = (cersai_checked == "Post-Sanction Only")
            due_diligence_critical_count = sum([
                valuation_collusive, legal_severe, rera_not_registered, cersai_post_sanction
            ])
            # Data-integrity check: years with employer cannot plausibly exceed
            # (age - minimum working age ~18). If it does, the application data
            # is internally inconsistent - a strong signal of fabricated/altered
            # employment history. Treat this the same as a critical document flag.
            if age_employer_inconsistent:
            ...
            # FOIR and new EMI already calculated above from inputs
            actual_credit_safe = actual_credit if actual_credit > 0 else 1
            effective_foir = foir  # auto-calculated: (existing EMI + new EMI) / actual income
            estimated_emi = _new_emi
            ...
            #  TIER-BASED CREDIT RISK ENGINE 
            # Tier functions and TIER_SCORES are module-level constants (defined above main())
            annual_actual = actual_credit_safe * 12
            lti = loan_amount / annual_actual if annual_actual > 0 else 0
            age_at_end = age + tenure

            # Employment-type-aware thresholds - self-employed has no fixed
            # retirement age (higher maturity cap) but needs a longer business
            # vintage; government has stronger job security (lower tenure min).
            is_self_employed = employment in ("Self-Employed Business", "Self-Employed Professional")
            AGE_CAP = 65 if is_self_employed else 60
            EMP_MIN_YEARS = 3 if is_self_employed else (1 if employment == "Government" else 2)
            emp_label = "business vintage" if is_self_employed else "years with employer"

            # (label, weight, tier, message)
            risk_components = [
                ("CIBIL score",         3.0, _t_cibil(cibil),
                    f"CIBIL is {cibil} (lender minimum: 700; below 650 is high risk)."),
                ("FOIR after this loan",3.0, _t_foir(effective_foir),
                    f"Total obligations: {effective_foir*100:.0f}% of actual income (RBI cap: 60%)."),
                ("Loan-to-value",       3.0, _t_ltv(ltv),
                    f"LTV is {ltv*100:.0f}% (RBI cap: 75-80% depending on loan size)."),
                ("EMI affordability",   2.5, _t_emi(emi_to_income),
                    f"New EMI is {emi_to_income*100:.0f}% of actual monthly income (safe: <=35%)."),
                ("Loan-to-income",      2.5, _t_lti(lti),
                    f"Loan is {lti:.1f}x annual income (safe limit: 5x)."),
                ("Income inflation",    2.5, _t_inflate(inflate),
                    f"Stated income is {inflate:.2f}x actual bank credits (safe: up to 1.2x)."),
                ("Employment stability",2.0, _t_emp(years_employer, employment),
                    f"{years_employer} year(s) with employer (minimum: {2 if employment not in ('Self-Employed Business','Self-Employed Professional') else 3} years for {employment.lower()})."),
                ("Age at maturity",     2.0, _t_age(age_at_end, employment),
                    f"Borrower will be {age_at_end} at loan maturity ({employment.lower()} limit: {65 if employment in ('Self-Employed Business','Self-Employed Professional') else 60})."),
                ("Credit enquiries",    1.5, _t_enq(enquiries),
                    f"{enquiries} enquiries in last 30 days (safe: <=2)."),
            ]

            tier_wscores = [(w, TIER_SCORES[t]) for _, w, t, _ in risk_components]
            total_weight = sum(w for w, _ in tier_wscores)
            worst_score  = max(r for _, r in tier_wscores)
            mean_score   = sum(w * r for w, r in tier_wscores) / total_weight
            credit_risk  = 0.60 * worst_score + 0.40 * mean_score

            n_critical = critical_flags_triggered
            n_minor    = max(flags_count_excl_oc - n_critical, 0)

            #  Doc risk: only critical flags go into the blend 
            # FIX 7: minor flags were being double-counted - once in doc_risk blend
            # and again as minor_flag_adder after floors. Now only critical flags
            # contribute to doc_risk blend; minor flags are additive AFTER floors only.
            critical_doc_risk = min(n_critical * 0.25, 1.0)
            minor_doc_risk    = min(n_minor * 0.04, 0.20)  # kept for named[] display only
            doc_risk = critical_doc_risk  # minor flags NOT included in blend

            #  Blend: 35% ML model + 45% credit rules (tier-based) + 20% critical doc risk 
            # Previously this only combined credit_risk and doc_risk - the ML model's
            # own fraud probability (`score`) was computed but never actually used here,
            # so due-diligence signals the model had learned (valuation agency, RERA,
            # legal opinion, etc.) never reached this final number. Fixed to match the
            # 35/45/20 blend already described in the comment above.
            overall_risk = 0.35 * score_for_blend + 0.45 * credit_risk + 0.20 * doc_risk
            overall_risk = min(max(overall_risk, 0.0), 1.0)

            #  FIX 8: OC/CC applied FIRST, before any floors 
            # Previously OC/CC ran after floors, allowing it to undercut them.
            # Now: apply protective reduction, then floors lock in minimum values.
            has_any_risk_flag = flags_count_excl_oc > 0
            oc_cc_active = bool(oc_cc) and not has_any_risk_flag
            if oc_cc_active:
                overall_risk = max(overall_risk - 0.04, 0.0)

            #  Hard policy floors - score MUST reflect every breach alert 
            # Using >= so boundary values (e.g. exactly 65% FOIR) are caught too

            # FOIR - RBI cap is 60%, so breach starts there
            if effective_foir >= 0.75:   overall_risk = max(overall_risk, 0.70)
            elif effective_foir >= 0.65: overall_risk = max(overall_risk, 0.55)
            elif effective_foir >= 0.60: overall_risk = max(overall_risk, 0.40)

            # LTV
            if ltv >= 0.80:   overall_risk = max(overall_risk, 0.55)
            elif ltv >= 0.75: overall_risk = max(overall_risk, 0.40)

            # Income inflation
            if inflate >= 2.0:   overall_risk = max(overall_risk, 0.70)
            elif inflate >= 1.5: overall_risk = max(overall_risk, 0.55)
            elif inflate >= 1.2: overall_risk = max(overall_risk, 0.40)

            # Loan-to-income
            if lti >= 6.0:   overall_risk = max(overall_risk, 0.55)
            elif lti >= 5.0: overall_risk = max(overall_risk, 0.40)

            # CIBIL
            if cibil < 600:   overall_risk = max(overall_risk, 0.70)
            elif cibil < 650: overall_risk = max(overall_risk, 0.55)
            elif cibil < 700: overall_risk = max(overall_risk, 0.40)

            # Employment (thresholds vary by employment type - see EMP_MIN_YEARS above)
            if years_employer == 0:
                overall_risk = max(overall_risk, 0.55)
            elif years_employer < EMP_MIN_YEARS * 0.5:
                overall_risk = max(overall_risk, 0.40)
            elif years_employer < EMP_MIN_YEARS:
                overall_risk = max(overall_risk, 0.25)

            # Age at maturity (cap varies by employment type - see AGE_CAP above)
            if age_at_end > AGE_CAP + 10:  overall_risk = max(overall_risk, 0.55)
            elif age_at_end > AGE_CAP + 5: overall_risk = max(overall_risk, 0.40)
            elif age_at_end > AGE_CAP:     overall_risk = max(overall_risk, 0.25)

            # Credit enquiries - checked against real fraud rates: 0-2 enquiries
            # ~3%, 3-4 ~5%, 5-6 ~8%, but 7+ is 100% fraud (10/10 and 9/9 cases in
            # the data - not "elevated", every single one). The old floor (0.55)
            # treated 7 enquiries the same as 4, massively understating this.
            if enquiries >= 7:   overall_risk = max(overall_risk, 0.85)
            elif enquiries > 3:  overall_risk = max(overall_risk, 0.40)
            elif enquiries > 2:  overall_risk = max(overall_risk, 0.25)

            # Age / employment tenure impossibility - internally inconsistent data
            # is treated as a critical fraud indicator, not a soft warning.
            if age_employer_inconsistent:
                overall_risk = max(overall_risk, 0.70)

            # Due-diligence findings - policy decision: even a single confirmed
            # critical finding (e.g. Collusive Valuer) should land at least in
            # Medium Risk, not Low-Medium - even though standalone data shows most
            # such loans (92.7%) are still clean, the bank's stance is that a
            # single confirmed critical finding warrants at least a Medium Risk
            # review, not just a soft "verify documents" nudge. Higher tiers
            # rescaled upward to keep the ordering strictly increasing.
            dd_floor = {0: 0.0, 1: 0.40, 2: 0.50, 3: 0.60}.get(due_diligence_critical_count, 0.70)
            overall_risk = max(overall_risk, dd_floor)
            overall_risk = min(overall_risk + due_diligence_critical_count * 0.04, 1.0)

            #  Critical flag floor + small guaranteed nudge - with credit bonus 
            # HYBRID fix, same reasoning as due-diligence above. Also removes a
            # double-count: critical_doc_risk already contributes to the 0.35/0.45/
            # 0.20 blend above (20% weight), so a pure floor+nudge here (not another
            # full additive stack) avoids counting the same flag twice.
            FLOOR_BASELINE = 0.35
            credit_bonus = max(0, credit_risk - FLOOR_BASELINE) * 0.5
            if critical_flags_triggered >= 2:
                critical_floor = 0.70
            elif critical_flags_triggered == 1:
                critical_floor = 0.55
            else:
                critical_floor = 0.0
            overall_risk = max(overall_risk, critical_floor + credit_bonus)
            overall_risk = min(overall_risk + critical_flags_triggered * 0.04, 1.0)

            #  Minor flags: additive on top of floors (no double-count) 
            # FIX: apply the 'any risk flag → min 0.40' floor FIRST,
            # then add minor flag adder ON TOP of that floor.
            # Previously the floor was after the adder, swallowing all minor flag increments.
            if has_any_risk_flag and overall_risk < 0.40:
                overall_risk = 0.40

            # Now add minor flags on top - each one visibly moves the score
            # 1 minor flag: +0.04, 2: +0.08, ..., cap at +0.20 (5 flags)
            minor_flag_adder = min(n_minor * 0.04, 0.20)
            overall_risk = min(overall_risk + minor_flag_adder, 1.0)

            #  Minor due-diligence nudge - keeps the score monotonic 
            # Full audit against real fraud rates (baseline 4.0%): Title_Chain_Clear=No
            # (3.69%), CERSAI_Checked=No (3.85%), and Legal_Opinion='Minor Observations'
            # (3.86%) all sit at or near baseline - essentially flat, so none get a
            # hard floor like Collusive Valuer or CERSAI='Post-Sanction Only'. But a
            # non-ideal selection should never be *free* to stack with zero effect -
            # 3+ flags should never tie with 2. Broker_Involved is deliberately NOT
            # included here - "Yes" (3.85%) is actually slightly SAFER than "No"
            # (4.17%) in the data, so nudging it would be dishonest, not just weak.
            minor_dd_adder = 0.0
            if title_chain == "No": minor_dd_adder += 0.05
            if cersai_checked == "No": minor_dd_adder += 0.05
            if legal_opinion == "Minor Observations": minor_dd_adder += 0.05
            overall_risk = min(overall_risk + minor_dd_adder, 1.0)
            ...
            if due_diligence_critical_count > 0:
                dd_reasons = []
                if valuation_collusive: dd_reasons.append("valuer flagged as collusive")
                if legal_severe: dd_reasons.append(f"legal opinion is '{legal_opinion}'")
                if rera_not_registered: dd_reasons.append("property not RERA registered")
                if cersai_post_sanction: dd_reasons.append("CERSAI checked only post-sanction")
                dd_display_score = {1: 0.25, 2: 0.40, 3: 0.55}.get(due_diligence_critical_count, 0.65)
                named.append(("Due-diligence findings", dd_display_score,
                    f"Underwriting file shows: {', '.join(dd_reasons)}. Historically this pattern correlates with elevated (not certain) fraud risk."))
            if title_chain == "No":
                named.append(("Title chain not clear", 0.15,
                    "Title chain has gaps - weak signal on its own, but adds to overall concern alongside other findings."))
            if n_minor > 0:
                named.append(("Minor flags", minor_doc_risk,
                    f"{n_minor} smaller suspicious flag(s) were raised."))
            if age_employer_inconsistent:
                named.append(("Age/employment inconsistency", 0.90,
                    f"{age}-year-old borrower cannot have {years_employer} year(s) at current employer - data is mathematically impossible."))
            # risk_components is now (label, weight, tier, message) - convert tier to score
            named.extend([(lbl, TIER_SCORES[t], msg) for lbl, _, t, msg in risk_components])

            top_reasons = sorted(named, key=lambda x: x[1], reverse=True)[:4]
            # Threshold 0.08 = anything above tier 0 (clean) shows up
            # This ensures minor flags, borderline FOIR, etc all appear in notes
            reasoning_lines = [msg for _, val, msg in top_reasons if val > 0.08]
            if not reasoning_lines:
                reasoning_lines = ["No significant risk factors detected. Profile appears clean across credit policy and documentation checks."]

            #  Policy breach notes - shown whenever a hard threshold is crossed 
            breach_notes = []

            # LTV
            if ltv > 0.80:
                breach_notes.append(f"LTV THRESHOLD BREACHED: {ltv*100:.0f}% exceeds RBI cap of 80% (75% for loans >Rs75L). Lender cannot legally disburse without regulatory exception.")
            elif ltv > 0.75:
                breach_notes.append(f"LTV WARNING: {ltv*100:.0f}% is above the 75% RBI cap applicable to loans >Rs75L. Verify loan size eligibility for this LTV.")

            # FOIR
            if foir >= 0.75:
                breach_notes.append(f"FOIR CRITICALLY HIGH: {foir*100:.0f}% - borrower is mathematically over-leveraged. RBI cap is 60%. Disbursement not advisable.")
            elif foir >= 0.65:
                breach_notes.append(f"FOIR SEVERELY BREACHED: {foir*100:.0f}% - well above RBI cap of 60%. Refer to risk team.")
            elif foir >= 0.60:
                breach_notes.append(f"FOIR THRESHOLD BREACHED: {foir*100:.0f}% exceeds RBI cap of 60%. Total obligations after this loan exceed safe lending limits.")

            # CIBIL
            if cibil < 600:
                breach_notes.append(f"CIBIL CRITICALLY LOW: {cibil} is below 600 - high default probability. Most lenders will not sanction below 650.")
            elif cibil < 650:
                breach_notes.append(f"CIBIL THRESHOLD BREACHED: {cibil} is below the 650 minimum. Requires senior underwriter sign-off to proceed.")
            elif cibil < 700:
                breach_notes.append(f"CIBIL BELOW PREFERRED: {cibil} is below the preferred 700 threshold. Lender may apply risk premium on rate.")

            # LTI
            if lti > 6.0:
                breach_notes.append(f"LOAN-TO-INCOME THRESHOLD BREACHED: {lti:.1f}x annual income - exceeds the 5x safe limit. Borrower is significantly over-borrowed.")
            elif lti > 5.0:
                breach_notes.append(f"LOAN-TO-INCOME ELEVATED: {lti:.1f}x annual income is above the 5x guideline. Review income sustainability.")

            # Income inflation
            if inflate > 2.0:
                breach_notes.append(f"INCOME INFLATION CRITICAL: Stated income is {inflate:.2f}x actual bank credits - likely document inflation. Refer for income re-verification.")
            elif inflate > 1.5:
                breach_notes.append(f"INCOME INFLATION THRESHOLD BREACHED: Stated income is {inflate:.2f}x actual bank credits. Exceeds 1.5x safe limit - independent verification required.")
            elif inflate > 1.2:
                breach_notes.append(f"INCOME INFLATION WARNING: Stated income is {inflate:.2f}x actual bank credits. Discrepancy warrants ITR and bank statement cross-check.")

            # Employment (thresholds depend on employment type - see EMP_MIN_YEARS)
            if years_employer == 0:
                breach_notes.append(f"EMPLOYMENT THRESHOLD BREACHED: 0 {emp_label}. Minimum requirement is {EMP_MIN_YEARS} year(s) for {employment.lower()} applicants.")
            elif years_employer < EMP_MIN_YEARS * 0.5:
                breach_notes.append(f"EMPLOYMENT THRESHOLD BREACHED: {years_employer} year(s) of {emp_label} - below the {EMP_MIN_YEARS*0.5:.1f}-year minimum. Confirm job/business stability and supporting documents.")
            elif years_employer < EMP_MIN_YEARS:
                breach_notes.append(f"EMPLOYMENT WARNING: {years_employer} year(s) of {emp_label} is below the preferred {EMP_MIN_YEARS}-year threshold for {employment.lower()} borrowers.")

            # Age at maturity (cap depends on employment type - see AGE_CAP)
            if age_at_end > AGE_CAP + 10:
                breach_notes.append(f"AGE AT MATURITY BREACHED: Borrower will be {age_at_end} at loan end. Lenders typically cap {employment.lower()} maturity at {AGE_CAP}.")
            elif age_at_end > AGE_CAP + 5:
                breach_notes.append(f"AGE AT MATURITY WARNING: Borrower will be {age_at_end} at loan end - above the {AGE_CAP}-year {employment.lower()} cap. Confirm continued income source.")
            elif age_at_end > AGE_CAP:
                breach_notes.append(f"AGE AT MATURITY ELEVATED: Borrower will be {age_at_end} at loan end. {employment} limit is {AGE_CAP} - verify post-retirement/continuing income source.")

            # Credit enquiries
            if enquiries >= 7:
                breach_notes.append(f"CREDIT ENQUIRIES CRITICAL: {enquiries} enquiries in 30 days - every case with 7+ enquiries in the historical data was confirmed fraud. Escalate immediately.")
            elif enquiries > 3:
                breach_notes.append(f"CREDIT ENQUIRIES ELEVATED: {enquiries} enquiries in last 30 days. Above 3 suggests active loan shopping - check for undisclosed liabilities.")

            # Age / employment tenure impossibility
            if age_employer_inconsistent:
                breach_notes.append(f"AGE/EMPLOYMENT TENURE IMPOSSIBLE: Borrower is {age} with {years_employer} year(s) at current employer - "
                                     f"this would require starting work at age {age - years_employer}. Application data is internally inconsistent; "
                                     f"treat as probable fabricated/altered employment history and refer for manual verification before any further processing.")

            # Critical flags
            if critical_flags_triggered > 0:
                crit_names = []
                if bank_tampered:   crit_names.append("Bank Statement Tampered")
                if pdf_anomaly:     crit_names.append("PDF Metadata Anomaly")
                if aml_flag:        crit_names.append("AML Flag")
                if pep:             crit_names.append("PEP Involved")
                if disb_nonseller:  crit_names.append("Disbursement to Non-Seller")
                if sig_mismatch:    crit_names.append("Signature Mismatch")
                if id_altered:      crit_names.append("ID Docs Altered")
                if overseas:        crit_names.append("Overseas Wire Transfer")
                if val_inflation:   crit_names.append("Valuation Inflation")
                breach_notes.append(f"CRITICAL FLAG(S) TRIGGERED: {', '.join(crit_names)}. Each of these alone is sufficient to block disbursement without compliance sign-off.")

            #  Anomaly checks: not scored, just worth a human glance 
            anomaly_notes = []
            if 0 < ltv < 0.30:
                anomaly_notes.append(f"LTV is unusually low at {ltv:.2f}. Large self-funded down payments are a known channel for money laundering - ask for source-of-funds proof.")
            if foir < 0.05:
                anomaly_notes.append(f"FOIR is unusually low at {foir:.2f}. Not a problem alone, but worth a cross-check alongside other numbers.")
            ...
            with r2:
                ...
                #  Risk notes: top reasons driving the score 
                if reasoning_lines:
                    notes_html = "".join([
                        f'<div style="margin-bottom:8px">'
                        f'<span style="font-size:0.92rem;color:#cbd5e1;line-height:1.5">{line}</span>'
                        f'</div>'
                        for line in reasoning_lines
                    ])
                    st.markdown(
                        f'<div style="background:#12151e;border:1px solid {color}33;border-radius:12px;'
                        f'padding:1rem 1.25rem;margin-top:0.5rem">'
                        f'<div style="font-size:0.68rem;font-weight:600;color:{color};text-transform:uppercase;'
                        f'letter-spacing:0.1em;margin-bottom:10px">RISK NOTES</div>'
                        f'{notes_html}</div>',
                        unsafe_allow_html=True
                    )
                ...
                if breach_notes:
                    breach_html = "".join([
                        f'<div style="margin-bottom:8px">'
                        f'<span style="font-size:0.92rem;color:#fca5a5;line-height:1.5">{note}</span>'
                        f'</div>'
                        for note in breach_notes
                    ])
                    st.markdown(
                        f'<div style="background:#1c0a0a;border:1px solid #f8717144;border-radius:12px;'
                        f'padding:1rem 1.25rem;margin-top:0.5rem">'
                        f'<div style="font-size:0.68rem;font-weight:600;color:#f87171;text-transform:uppercase;'
                        f'letter-spacing:0.1em;margin-bottom:10px">POLICY BREACH ALERTS</div>'
                        f'{breach_html}</div>',
                        unsafe_allow_html=True
                    )
            ...
            # inflate already computed above (line ~490) - no need to recompute
            # flags_count must NOT include oc_cc - it is protective, not a risk flag
            flags_count = flags_count_excl_oc

            st.markdown("<br>", unsafe_allow_html=True)
            s1, s2, s3 = st.columns(3)
            with s1:
                col = "#f87171" if inflate > 1.5 else "#fbbf24" if inflate > 1.2 else "#34d399"
    ...
    # render advisor even after rerun if score exists in session state
        if not submitted and all(k in st.session_state for k in ['last_inputs','last_score','last_inflate','last_flags','last_shap','last_overall']):
            st.markdown("<br>", unsafe_allow_html=True)
            render_chat_widget(
                st.session_state['last_inputs'],
                st.session_state['last_score'],
                st.session_state['last_inflate'],
                st.session_state['last_flags'],
                st.session_state['last_shap'],
                overall_risk=st.session_state['last_overall']
            )
    ...
    def page_kyc_verification():
        st.markdown("### KYC Verification")
        st.markdown('<p style="color:#8899aa;font-size:0.9rem">Upload borrower identity documents for automated OCR extraction and verification.</p>', unsafe_allow_html=True)

        kyc_tab1, kyc_tab2 = st.tabs(["PAN Verification", "Aadhaar Verification"])

        #  PAN Verification with OCR 
        with kyc_tab1:
            st.markdown("#### PAN Card Verification")
            st.markdown('<p style="color:#8899aa;font-size:0.85rem">Upload a PAN card image — details will be extracted automatically.</p>', unsafe_allow_html=True)

            uploaded_pan = st.file_uploader("Upload PAN Card Image", type=["jpg", "jpeg", "png"], key="pan_uploader")

            if uploaded_pan:
                st.image(uploaded_pan, caption="Uploaded PAN Card", use_column_width=True)
                with st.spinner("Extracting details from PAN card..."):
                    import time
                    image_bytes = uploaded_pan.read()
                    ocr_text, err = extract_text_from_image(image_bytes)

                if err:
                    st.error(f"OCR failed: {err}")
                elif ocr_text:
                    pan_extracted, name_extracted, dob_extracted = parse_pan_from_text(ocr_text)
                    st.markdown("**Extracted Details — confirm before verifying:**")
                    pan_col1, pan_col2 = st.columns([2, 1])
                    with pan_col1:
                        pan_input = st.text_input("PAN Number", value=pan_extracted or "", max_chars=10)
                    with pan_col2:
                        pan_name = st.text_input("Name as per PAN", value=name_extracted or "")
                    pan_dob = st.text_input("Date of Birth", value=dob_extracted or "", help="Format: DD/MM/YYYY")

                    verify_pan_btn = st.button("Verify PAN", use_container_width=True, key="verify_pan_btn")

                    if verify_pan_btn:
                        if not pan_input or len(pan_input) != 10:
                            st.warning("Please enter a valid 10-character PAN number.")
                        elif not pan_name or not pan_dob:
                            st.warning("Please enter name and date of birth.")
                        else:
                            with st.spinner("Verifying PAN..."):
                                time.sleep(1.5)
                                st.markdown(f"""
                                <div style="background:#064e3b;border:1px solid #34d399;border-radius:12px;padding:1.25rem 1.5rem;margin:1rem 0">
                                    <div style="font-size:1rem;font-weight:600;color:#34d399;margin-bottom:0.5rem"> PAN Verified Successfully</div>
                                    <div style="color:#6ee7b7;font-size:0.9rem">PAN: <strong style="color:#f0f4f8">{pan_input.upper()}</strong></div>
                                    <div style="color:#6ee7b7;font-size:0.9rem;margin-top:4px">Name: <strong style="color:#f0f4f8">{pan_name}</strong></div>
                                    <div style="color:#6ee7b7;font-size:0.9rem;margin-top:4px">Date of Birth: <strong style="color:#f0f4f8">{pan_dob}</strong></div>
                                    <div style="color:#6ee7b7;font-size:0.9rem;margin-top:4px">Category: <strong style="color:#f0f4f8">Individual</strong></div>
                                    <div style="color:#6ee7b7;font-size:0.9rem;margin-top:4px">Status: <strong style="color:#f0f4f8">Valid</strong></div>
                                </div>""", unsafe_allow_html=True)
            else:
                st.markdown("""
                <div style="background:#1a1f2e;border:1px dashed #2d3748;border-radius:12px;padding:2rem;text-align:center;margin-top:1rem">
                    <div style="color:#8899aa;font-size:0.9rem">Upload a PAN card image to auto-extract and verify details</div>
                </div>""", unsafe_allow_html=True)

        #  Aadhaar Verification (mock) 
        with kyc_tab2:
            st.markdown("#### Aadhaar OTP Verification")
            st.markdown('<p style="color:#8899aa;font-size:0.85rem">Step 1: Enter Aadhaar number to send OTP to borrower\'s registered mobile. Step 2: Enter OTP to complete verification.</p>', unsafe_allow_html=True)

            if "aadhaar_ref_id" not in st.session_state:
                st.session_state.aadhaar_ref_id = None
            if "aadhaar_verified" not in st.session_state:
                st.session_state.aadhaar_verified = False

            st.markdown("**Step 1 — Send OTP**")
            aadh_col1, aadh_col2 = st.columns([2, 1])
            with aadh_col1:
                aadhaar_input = st.text_input("Aadhaar Number", max_chars=12,
                                              help="12-digit Aadhaar number")
            with aadh_col2:
                st.markdown("<br>", unsafe_allow_html=True)
                send_otp_btn = st.button("Send OTP", use_container_width=True, key="send_otp_btn")
            ...
            if send_otp_btn:
                ...
                if len(aadhaar_clean) != 12 or not aadhaar_clean.isdigit():
                    ...
                    with st.spinner("Sending OTP to registered mobile..."):
                        import time
                        time.sleep(1.5)
                        st.session_state.aadhaar_ref_id = "ref_" + aadhaar_clean
                        st.success("OTP sent successfully to the Aadhaar-registered mobile number.")

            if st.session_state.aadhaar_ref_id:
                st.markdown("---")
                ...
                with otp_col2:
                    st.markdown("<br>", unsafe_allow_html=True)
                    verify_otp_btn = st.button("Verify OTP", use_container_width=True, key="verify_otp_btn")
                ...
                if verify_otp_btn:
                    if not otp_input or len(otp_input) != 6:
                        ...
                        with st.spinner("Verifying OTP..."):
                            ...
                            st.markdown("""
                            <div style="background:#064e3b;border:1px solid #34d399;border-radius:12px;padding:1.25rem 1.5rem;margin:1rem 0">
                                <div style="font-size:1rem;font-weight:600;color:#34d399;margin-bottom:0.75rem"> Aadhaar Verified Successfully</div>
                                <div style="color:#6ee7b7;font-size:0.9rem">Status: <strong style="color:#f0f4f8">Identity Confirmed</strong></div>
                                <div style="color:#6ee7b7;font-size:0.9rem;margin-top:4px">KYC: <strong style="color:#f0f4f8">Complete</strong></div>
                            </div>""", unsafe_allow_html=True)
                            st.session_state.aadhaar_ref_id = None
    ...
    pages = [
        st.Page(page_model_performance, title="Model Performance"),
        st.Page(page_score_application, title="Score New Application"),
        st.Page(page_feature_importance, title="Feature Importance"),
        st.Page(page_inspect_predictions, title="Inspect Predictions"),
        st.Page(page_model_comparison, title="Model Comparison"),
        st.Page(page_kyc_verification, title="KYC Verification"),
    ]
