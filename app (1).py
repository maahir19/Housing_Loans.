...
Divided by actual income: Rs{actual_credit:,}/mo
        ...
        if submitted:
            ...
            # thin sample - the 4-flag tier is judgment-extrapolated, not tightly
            # data-fit). Each additional finding must strictly increase the floor -
            # uses the same tier boundaries (0.25/0.40/0.55) the app already shows
            # as Low-Medium/Medium/High, so escalation lands in a consistent tier.
            if due_diligence_critical_count >= 4:
                overall_risk = max(overall_risk, 0.65)
            elif due_diligence_critical_count == 3:
                overall_risk = max(overall_risk, 0.55)
            elif due_diligence_critical_count == 2:
                overall_risk = max(overall_risk, 0.40)
            elif due_diligence_critical_count == 1:
                overall_risk = max(overall_risk, 0.25)

            #  Critical flag floor - with credit bonus 
            # FIX: instead of hard max(overall, floor) which collapses everything
            # to exactly 0.55, we add a credit_bonus above the floor so bad profiles
            # score higher than clean ones even when both have a critical flag.
            # Formula: floor + 0.5 * max(0, credit_risk - 0.35)
            # A clean profile + 1 flag stays at 0.55.
            # A bad credit profile (credit_risk=0.68) + 1 flag = 0.55 + 0.5*(0.68-0.35) = 0.715
            FLOOR_BASELINE = 0.35
